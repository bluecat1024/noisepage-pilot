...
def generate_per_query_plots(query_stream, dir_output):
    ...
    for group in qid_groups:
        ...
        group[1].plot(title=f"qid: {group[0]}", x="query_order", y="elapsed_us", color='r', ax=ax, kind='scatter')
        group[1].plot(title=f"qid: {group[0]}", x="query_order", y="pred_elapsed_us", color='b', ax=ax, kind='scatter')
        ax.set_xticks([])
